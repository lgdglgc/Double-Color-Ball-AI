...
请先使用以下真实开奖数据进行预处理统计（若数据不足，请说明）：
```
{lottery_history}
```
（格式为数组，每项包含 period, red_balls[], blue_ball）
...
在开始预测前，请先完成以下统计分析（必须使用提供的 {lottery_history} 数据，输出 JSON 统计结果）：
...
### 3. 趋势分析
- 计算每个红球的趋势分数：`(超短期频率 - 中期频率) × 50 + (短期频率 - 中期频率) × 25`
...
**步骤 1: 计算加权频率得分**
```
每个红球的得分 = (最近5期出现次数 × 5) + (最近10期出现次数 × 3) + (最近30期出现次数 × 2)
```
...
#### 输出要求
在 `description` 中说明最核心的数据依据（限 40 字以内，如：“红球加权频次Top；区间1-3-2；蓝球07频次高”）。
...
**步骤 1: 计算遗漏加权得分**
```
每个红球的得分 = if 遗漏期数 in [5, 10]: 遗漏期数 × 1.0 elif 遗漏期数 in [11, 20]: 遗漏期数 × 1.5 else: 遗漏期数 × 0.8
```
...
**步骤 4: 最终选择**
- 从得分最高的 12 个候选号码中，选择满足所有硬约束 of 6 个
...
#### 输出要求
在 `description` 中说明最核心的数据依据（限 40 字以内，如：“红球以高遗漏冷号为主（如12遗漏14期）；蓝球02为中遗漏冷号”）。
...
#### 输出要求
在 `description` 中说明最核心的数据依据（限 40 字以内，如：“红球奇偶大小比均为3:3，均值偏向105；无连号，蓝球10为中频”）。
...
**步骤 2: 趋势强度评分**
```
每个红球的趋势分 = (超短期频率 - 中期频率) × 50 + (短期频率 - 中期频率) × 25
```
...
#### 输出要求
在 `description` 中说明最核心的数据依据（限 40 字以内，如：“选中09（趋势分+45）、16（+38）等上行红球；蓝球04最接近均值”）。
...
**步骤 2: 加权综合得分**
```
每个红球的综合得分 = 热号得分 × 0.30 + 冷号得分 × 0.25 + 平衡得分 × 0.20 + 周期得分 × 0.25
```
...
**蓝球综合评分**
```
每个蓝球的综合得分 = 热度得分（最近20期频率排名） × 0.30 + 遗漏得分（遗漏期数/最大遗漏） × 0.30 + 周期得分（是否接近平均遗漏期） × 0.20 + 中频得分（是否为中频蓝球） × 0.20
```
选择综合得分最高的蓝球。

#### 输出要求
在 `description` 中说明最核心的数据依据（限 40 字以内，如：“红球09（综合分82分）、12（76分）多维指标最高，蓝球为各维度高分”）。
...
{{
  ...
  "predictions": [
    {{
      ...
      "blue_ball": "XX",
      "description": "基于5期加权频率，选择09(5期3次)、16(5期2次)等高频号；区间分布1-3-2；蓝球07(20期内4次)；总和115"
    }},
    {{
      "group_id": 2,
      "strategy": "冷号逆向者",
    ...
    {{
      ...
      "red_balls": ["XX", "XX", "XX", "XX", "XX", "XX"],
      "blue_ball": "XX",
      "description": "中频号为主，奇偶3:3，大小3:3；总和112；无连号；区间分布1-2-3；蓝球10(中频)"
    }},
    {{
      "group_id": 4,
      "strategy": "周期理论家",
...
- **description 长度限制**：必须高度精简，控制在 40 字以内（包含标点）。只写最核心的量化特征（如奇偶大小比、总和及蓝球遗漏），不要写多余的叙述或修饰语。
- 必须包含 `statistics` 字段
- `error` 字段（若数据不足或约束无法满足）：null
...
1. **严格遵循量化方法**：所有得分、权重、硬约束都已明确定义，请严格执行
2. **数据驱动决策**：每个选择都应基于统计数据，而非主观判断
3. **约束优先级**：硬约束（如奇偶比）必须满足，软约束尽量满足
4. **透明度要求**：`description` 必须说明选号依据，便于后续分析

**Prompt 版本**: v2.0 修正版
**更新日期**: 2026-07-07
**主要改进**: 强制真实数据依赖、趋势公式标准化、区间硬约束、约束优先级矩阵、评分可执行伪代码、增加 statistics 字段与约束验证

**测试指令**（在实际运行时可参考）：请使用模拟数据（近100期随机生成）验证5个策略是否符合所有硬约束，并返回JSON + 约束验证通过/失败列表。
